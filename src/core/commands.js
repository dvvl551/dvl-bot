...
const CATEGORY_ORDER = ['Config', 'General', 'Info', 'Utility', 'Automation', 'Voice', 'Roles', 'Permissions', 'Welcome', 'Logs', 'Support', 'Giveaway', 'TikTok', 'Moderation', 'Security', 'Owner', 'Fun'];

const CONFIG_HELP_GROUPS = {
  'Core setup': ['config', 'dashboard', 'configpanel', 'setprefix', 'setembedcolor', 'setlanguage'],
  'Channel restrictions': ['piconly', 'piconlylist', 'ghostping', 'automodignore', 'automodunignore', 'automodignored'],
  'Welcome / Leave': ['welcome', 'leave', 'welcometoggle', 'setwelcomechannel', 'setwelcometitle', 'setwelcomemessage', 'leavetoggle', 'setleavechannel', 'setleavetitle', 'setleavemessage', 'previewwelcome', 'previewleave', 'leavedm', 'setleavedm', 'testleavedm', 'welcomeconfig'],
  'Logs / routing / boost': ['logs', 'boost', 'setlogchannel', 'logtoggle', 'logtype', 'logconfig', 'boosttoggle', 'setboostchannel', 'setboosttitle', 'setboostmessage', 'previewboost', 'boostconfig', 'invites', 'inviteleaderboard'],
  'Roles / access': ['autorole', 'roleall', 'autoroleadd', 'autoroleremove', 'autorolelist', 'reactbutton', 'setstatusrole', 'statusroletoggle', 'statusrolecheck', 'statusroleconfig'],
  'Voice tools': ['createvoc', 'voicepanel', 'move', 'moveall', 'pull', 'voicekick', 'voicemute', 'voiceunmute', 'voiceban', 'voiceunban', 'setvoicemuterole', 'setvoicebanrole', 'voicepermconfig'],
  'Server stats': ['statssetup', 'setstatslabel', 'statsrefresh', 'statsconfig', 'statsoff'],
  'Permission levels': ['permrole', 'permcmd', 'permconfig'],
  'Support relay': ['supporttoggle', 'setsupportchannel', 'setsupportrole', 'support', 'reply'],
  'TikTok': ['tiktokadd', 'tiktokremove', 'tiktoklist', 'tiktoksetrole', 'tiktoksetchannel', 'tiktoktogglelive', 'tiktoktogglevideo', 'tiktokforcecheck', 'tiktoktest'],
  'Progress board': ['trophy', 'trophychannel', 'trophyrefresh', 'trophyconfig', 'trophygoal', 'trophytitle', 'trophyimage', 'milestonerole', 'milestoneinterval', 'milestonechannel', 'milestoneconfig'],
  'Security / AutoMod': ['automodconfig', 'securitypreset', 'setantispam', 'setantimention', 'setanticaps', 'setantiemoji', 'setraidage'],
  'Sticky messages': ['stickyset', 'stickyoff', 'stickyconfig'],
  'Automation / auto react': ['autoreact'],
  'Diagnostics / quality': ['setupcheck'],
  'Backups': ['backup']
};
...
function createLogsOverviewEmbed(guildConfig, prefix = '+') {
  ...
  const families = LOG_CHANNEL_GROUPS.map((group) => {
  ...
  const enabledCount = LOG_TYPE_CHOICES.filter(([type]) => logs.types?.[type] !== false).length;
  const disabledLines = LOG_TYPE_CHOICES.filter(([type]) => logs.types?.[type] === false).map(([type]) => `• ${getLogTypeLabel(type)}`);

  const embed = baseEmbed(guildConfig, '🧾 Logs overview', [
    `**Master:** ${logs.enabled ? 'on' : 'off'}`,
    `**Default route:** ${defaultChannelId ? `<#${defaultChannelId}>` : 'not set'}`,
    `**Active event types:** ${enabledCount}/${LOG_TYPE_CHOICES.length}`,
    '',
    `Shortcuts: \`${prefix}logs boost here\` • \`${prefix}logs view\` • \`${prefix}logs types\` • \`${prefix}logs test boost\``
  ].join('\n'));

  embed.addFields({ name: 'Families', value: families.join('\n').slice(0, 1024), inline: false });
  ...
  if (disabledLines.length) {
    const chunks = splitLinesForField(disabledLines, 14);
...
function getCommandAttachmentUrls(ctx) {
  ...
  return [...(ctx.message.attachments?.values?.() || [])].map((attachment) => attachment.url).filter(Boolean);
}

function getCommandImageUrl(ctx) {
  return getCommandAttachmentUrls(ctx).find((url) => /\.(png|jpe?g|gif|webp|bmp)(\?.*)?$/i.test(String(url)));
}
...
async function resolveGiveawayEntry(ctx, rawInput) {
  ...
  if (messageId) {
    const found = Object.values(giveaways).find((entry) => entry?.messageId === messageId || entry?.id === messageId);
    if (found) return { id: found.id, giveaway: found };
  }
  const normalized = raw.toLowerCase();
  const found = Object.values(giveaways).find((entry) => String(entry?.id || '').toLowerCase() == normalized || String(entry?.prize || '').toLowerCase() == normalized);
  return found ? { id: found.id, giveaway: found } : { id: raw, giveaway: null };
}
...
function buildWelcomeModuleEmbed(guildConfig) {
  const welcome = guildConfig.welcome || {};
  return baseEmbed(guildConfig, '👋 Welcome module', [
    `**Status:** ${welcome.enabled ? 'on' : 'off'}`,
    `**Channel:** ${welcome.channelId ? `<#${welcome.channelId}>` : 'not set'}`,
    `**Title:** ${welcome.title ? `\`${welcome.title}\`` : 'default'}`,
    `**Message:** ${welcome.message ? `\`${welcome.message}\`` : 'not set'}`,
    '',
    '**Quick use:**',
    '`+welcome on` • `+welcome channel #welcome` • `+welcome message Welcome {user}` • `+welcome test`'
  ].join('\n').slice(0, 4000));
}

function buildLeaveModuleEmbed(guildConfig) {
  const leave = guildConfig.leave || {};
  return baseEmbed(guildConfig, '👋 Leave module', [
    `**Leave channel:** ${leave.enabled ? 'on' : 'off'} • ${leave.channelId ? `<#${leave.channelId}>` : 'not set'}`,
    `**Leave title:** ${leave.title ? `\`${leave.title}\`` : 'default'}`,
    `**Leave message:** ${leave.message ? `\`${leave.message}\`` : 'not set'}`,
    '',
    `**Leave DM:** ${leave.dmEnabled ? 'on' : 'off'}`,
    `**DM title:** ${leave.dmTitle ? `\`${leave.dmTitle}\`` : 'default'}`,
    `**DM message:** ${leave.dmMessage ? `\`${leave.dmMessage}\`` : 'not set'}`,
    '',
    '**Quick use:**',
    '`+leave on` • `+leave channel #logs` • `+leave dm on` • `+leave dmmessage Tu as quitté {server}` • `+leave testdm`'
  ].join('\n').slice(0, 4000));
}

function buildBoostModuleEmbed(guildConfig) {
  const boost = guildConfig.boost || {};
  return baseEmbed(guildConfig, '🚀 Boost module', [
    `**Status:** ${boost.enabled ? 'on' : 'off'}`,
    `**Channel:** ${boost.channelId ? `<#${boost.channelId}>` : 'not set'}`,
    `**Title:** ${boost.title ? `\`${boost.title}\`` : 'default'}`,
    `**Message:** ${boost.message ? `\`${boost.message}\`` : 'not set'}`,
    '',
    '**Quick use:**',
    '`+boost on` • `+boost channel #boosts` • `+boost title 🚀 Nouveau boost` • `+boost test`'
  ].join('\n').slice(0, 4000));
}
...
function createHelpEmbed(client, guildConfig, target = 'Home', page = 1) {
  ...
  if (category === 'Home') {
    ...
    embed
      ...
      .setDescription([
        ...
        '',
        `Examples: \`${prefix}help ban\`, \`${prefix}help logs\`, \`${prefix}help support\`, \`${prefix}help setup\`, \`${prefix}help all\`, \`${prefix}dashboard\``,
        `Prefix: \`${prefix}\``,
  ...
  if (category === 'Setup') {
    ...
    embed
      ...
      .setDescription([
        'Main setup hub for the server.',
        `Fast start: \`${prefix}config\` • \`${prefix}logs\` • \`${prefix}help support\` • \`${prefix}help permissions\``,
  ...
  const commands = [
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        const embed = baseEmbed(ctx.guildConfig, '⚙️ Guild Config', 'Main server setup overview. Use `+help setup` for the setup hub.').addFields(
          {
            name: 'Core',
            value: [
              `**Prefix:** \`${g.prefix}\``,
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '🧩 Modules overview', [
          ...lines,
          '',
          `Quick use: \`${ctx.prefix}logs view\` • \`${ctx.prefix}ghostping view\` • \`${ctx.prefix}stats view\` • \`${ctx.prefix}trophy config\` • \`${ctx.prefix}autorole view\` • \`${ctx.prefix}autoreact config\``
        ].join('\n'))] });
      }
    }),
    ...
    makeSimpleCommand({
      name: 'logs',
      aliases: ['logpanel', 'logssetup', 'setuplogs'],
      category: 'Logs',
      description: 'Logs hub: routes, enable state, event list, tests, or the old panel',
      usage: 'logs [view|types|panel|test <type|all>|on|off|enable <types...>|disable <types...>|<type|family> <here|#channel|off>]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageGuild],
      async execute(ctx) {
        ...
        if (!sub || ['view', 'config', 'show', 'list'].includes(sub)) {
...
Route resolved to ${channel}.`)] }).catch(() => null);
        ...
        return ctx.invalidUsage(`Examples: \`${ctx.prefix}logs view\`, \`${ctx.prefix}logs boost here\`, \`${ctx.prefix}logs enable join leave boost\`, \`${ctx.prefix}logs test boost\`.`);
      }
    }),
...
Route resolved to ${channel}.`)] }).catch(() => null);
    ...
    makeSimpleCommand({
      name: 'roles',
      aliases: ['rolehub', 'rolesetup'],
      category: 'Roles',
      description: 'Simple role hub: view setup, give/remove a role, or open mass-role shortcuts',
      usage: 'roles <view|add|remove|autorole|all> [@user] [@role]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageRoles],
      async execute(ctx) {
        const sub = String(ctx.args[0] || 'view').toLowerCase();
        if (['view', 'list', 'show'].includes(sub)) {
          ...
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '🎭 Roles hub', [
            ...
            `• \`${ctx.prefix}roleall add @Event humans\``
          ].join('\n'))] });
        }
        ...
        if (!target || !role) return ctx.invalidUsage('Example: `+roles add @user @role`');
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        let scope = String(ctx.getText('scope', 2) || '').toLowerCase();
        if (!action && ['addroleall', 'giveroleall'].includes(ctx.command.name)) action = 'add';
        if (!action && ['removeroleall', 'takeallrole'].includes(ctx.command.name)) action = 'remove';
        if (!scope && ['all', 'humans', 'bots'].includes(String(ctx.args[2] || '').toLowerCase())) scope = String(ctx.args[2] || '').toLowerCase();
        if (!scope) scope = 'all';

        const roleArgIndex = ['addroleall', 'giveroleall', 'removeroleall', 'takeallrole'].includes(ctx.command.name) ? 0 : 1;
        const role = await ctx.getRole('role', roleArgIndex);
        if (!['add', 'remove'].includes(action) || !role || !['all', 'humans', 'bots'].includes(scope)) {
        ...
        if (!me?.permissions.has(PermissionFlagsBits.ManageRoles)) {
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '⛔ Role all', 'I need **Manage Roles** to do that.')] });
        }
        if (role.managed || role.id === ctx.guild.id) {
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '⛔ Role all', 'That role cannot be mass managed.')] });
        }
        if (role.position >= me.roles.highest.position) {
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '⛔ Role all', 'Move the target role below my highest role first.')] });
        }
        if (ctx.member && role.position >= ctx.member.roles.highest.position && ctx.guild.ownerId !== ctx.user.id) {
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '⛔ Role all', 'That role is above or equal to your highest role.')] });
        }
    ...
    makeSimpleCommand({
      name: 'backup',
      aliases: ['backupconfig', 'serverbackup', 'bk'],
      category: 'Config',
      description: 'Create, import, list, restore, export or delete server backups',
      usage: 'backup <create|import|list|info|restore|export|delete> [id] [name]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageGuild],
      slash: {
        root: 'utility',
      ...
      async execute(ctx) {
        const action = String(ctx.getText('action', 0) || 'create').toLowerCase();
        const rawId = String(ctx.getText('id', 1) || '').trim();
        const backupId = rawId.toLowerCase();
...
↳ ${when} • from **${entry.sourceGuildName || 'Unknown server'}** • ${entry.meta?.channels || 0} ch • ${entry.meta?.roles || 0} roles`;
        ...
        if (!backupId) return ctx.invalidUsage(`Try \`${ctx.prefix}backup load <id>\`, \`${ctx.prefix}backup export <id>\` or \`${ctx.prefix}backup list\`.`);

        const found = backups.find((entry) => String(entry.id || '').toLowerCase() === backupId || String(entry.name || '').toLowerCase() === backupId);
        if (!found) return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '💾 Backup', `No global backup found with id or name \`${rawId}\`.`)] });

        if (['info', 'show'].includes(action)) {
...
Reason: **${reason}**`)] });
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        if (['view', 'config', 'show', 'list'].includes(action)) {
...
Matched: **${result.matched}**.`)] });
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        const textValue = fillTemplate(ctx.guildConfig.welcome.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.welcome.title || '👋 Welcome', textValue)] });
      }
    }),
    makeSimpleCommand({
      name: 'setleavetitle',
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        const member = await ctx.getMember('target', 0) || ctx.member;
        const textValue = fillTemplate(ctx.guildConfig.leave.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.leave.title || '👋 Member left', textValue)] });
      }
    }),
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        const member = await ctx.getMember('target', 0) || ctx.member;
        const textValue = fillTemplate(ctx.guildConfig.boost.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, boostCount: ctx.guild.premiumSubscriptionCount || 0, boostTier: ctx.guild.premiumTier || 0 });
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.boost.title || '🚀 New boost', textValue)] });
      }
    }),
    makeSimpleCommand({
      name: 'boostconfig',
      ...
      async execute(ctx) {
        ...
        const desc = [
          ...
          `**Boost title:** ${b.title ? `\`${b.title}\`` : 'default'}`,
          `**Boost message:** ${b.message ? `\`${b.message}\`` : 'not set'}`
        ].join('\n');
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        const desc = [
          `**Welcome:** ${w.enabled ? 'on' : 'off'} • ${w.channelId ? `<#${w.channelId}>` : 'no channel'}`,
          `**Welcome title:** ${w.title ? `\`${w.title}\`` : 'default'}`,
          `**Welcome message:** ${w.message ? `\`${w.message}\`` : 'not set'}`,
          '',
          `**Leave:** ${l.enabled ? 'on' : 'off'} • ${l.channelId ? `<#${l.channelId}>` : 'no channel'}`,
          `**Leave title:** ${l.title ? `\`${l.title}\`` : 'default'}`,
          `**Leave message:** ${l.message ? `\`${l.message}\`` : 'not set'}`,
          `**Leave DM:** ${l.dmEnabled ? 'on' : 'off'}`,
          `**Leave DM title:** ${l.dmTitle ? `\`${l.dmTitle}\`` : 'default'}`,
          `**Leave DM message:** ${l.dmMessage ? `\`${l.dmMessage}\`` : 'not set'}`,
          '',
          `**Boost:** ${b.enabled ? 'on' : 'off'} • ${b.channelId ? `<#${b.channelId}>` : 'no channel'}`,
          `**Boost title:** ${b.title ? `\`${b.title}\`` : 'default'}`,
          `**Boost message:** ${b.message ? `\`${b.message}\`` : 'not set'}`
        ].join('\n');
    ...
    makeSimpleCommand({
      name: 'welcome',
      aliases: ['welcomehub', 'welcomesetup'],
      category: 'Welcome',
      description: 'Clean welcome module hub',
      usage: 'welcome [view|on|off|channel <here|#channel>|title <text>|message <text>|test [@member]]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageGuild],
      async execute(ctx) {
        const action = String(ctx.args[0] || 'view').toLowerCase();
        if (['view', 'config', 'show', 'list'].includes(action)) return ctx.reply({ embeds: [buildWelcomeModuleEmbed(ctx.guildConfig)] });
        if (action === 'on' || action === 'off') {
        ...
        if (action === 'title') {
          ...
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}welcome title 👋 Bienvenue\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.welcome.title = value; return guild; });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '👋 Welcome title', `Welcome title set to **${value}**.`)] });
        }
        if (action === 'message') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}welcome message Bienvenue {user} sur {server}\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.welcome.message = value; return guild; });
        ...
        if (action === 'test' || action === 'preview') {
          ...
          const textValue = fillTemplate(ctx.guildConfig.welcome.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.welcome.title || '👋 Welcome', textValue)] });
        }
        return ctx.invalidUsage(`Examples: \`${ctx.prefix}welcome view\`, \`${ctx.prefix}welcome on\`, \`${ctx.prefix}welcome channel #welcome\`, \`${ctx.prefix}welcome message Bienvenue {user}\`.`);
      }
    }),

    makeSimpleCommand({
      name: 'leave',
      aliases: ['leavehub', 'goodbyehub'],
      category: 'Welcome',
      description: 'Clean leave module hub with leave DM support',
      usage: 'leave [view|on|off|channel <here|#channel>|title <text>|message <text>|dm on|off|dmtitle <text>|dmmessage <text>|test|testdm [@member]]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageGuild],
      async execute(ctx) {
        const action = String(ctx.args[0] || 'view').toLowerCase();
        if (['view', 'config', 'show', 'list'].includes(action)) return ctx.reply({ embeds: [buildLeaveModuleEmbed(ctx.guildConfig)] });
        if (action === 'on' || action === 'off') {
        ...
        if (action === 'title') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}leave title 👋 Un membre a quitté\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.leave.title = value; return guild; });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '👋 Leave title', `Leave title set to **${value}**.`)] });
        }
        if (action === 'message') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}leave message {userTag} a quitté {server}\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.leave.message = value; return guild; });
        ...
        if (action === 'dmtitle') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}leave dmtitle 👋 Tu as quitté {server}\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.leave.dmTitle = value; return guild; });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '👋 Leave DM title', `Leave DM title set to **${value}**.`)] });
        }
        if (action === 'dmmessage') {
          const value = (ctx.getRest(1) || '').trim();
        ...
        if (action === 'test' || action === 'preview') {
          const member = await ctx.getMember('target', 1) || ctx.member;
          const textValue = fillTemplate(ctx.guildConfig.leave.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.leave.title || '👋 Member left', textValue)] });
        }
        if (action === 'testdm') {
          const member = await ctx.getMember('target', 1) || ctx.member;
          const title = fillTemplate(ctx.guildConfig.leave.dmTitle || '👋 You left {server}', { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
          const textValue = fillTemplate(ctx.guildConfig.leave.dmMessage || 'You left **{server}**. You can always come back with a fresh start.', { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, memberCount: ctx.guild.memberCount });
          const sent = await member.send({ embeds: [baseEmbed(ctx.guildConfig, title, textValue)] }).catch(() => null);
        ...
        return ctx.invalidUsage(`Examples: \`${ctx.prefix}leave view\`, \`${ctx.prefix}leave on\`, \`${ctx.prefix}leave dm on\`, \`${ctx.prefix}leave dmmessage Tu as quitté {server}\`, \`${ctx.prefix}leave testdm\`.`);
      }
    }),

    makeSimpleCommand({
      name: 'boost',
      aliases: ['boosthub', 'boostsetup'],
      category: 'Logs',
      description: 'Clean boost module hub',
      usage: 'boost [view|on|off|channel <here|#channel>|title <text>|message <text>|test [@member]]',
      guildOnly: true,
      userPermissions: [PermissionFlagsBits.ManageGuild],
      async execute(ctx) {
        const action = String(ctx.args[0] || 'view').toLowerCase();
        if (['view', 'config', 'show', 'list'].includes(action)) return ctx.reply({ embeds: [buildBoostModuleEmbed(ctx.guildConfig)] });
        if (action === 'on' || action === 'off') {
        ...
        if (action === 'title') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}boost title 🚀 Nouveau boost\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.boost.title = value; return guild; });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '🚀 Boost title', `Boost title set to **${value}**.`)] });
        }
        if (action === 'message') {
          const value = (ctx.getRest(1) || '').trim();
          if (!value) return ctx.invalidUsage(`Example: \`${ctx.prefix}boost message {user} a boost le serveur\`.`);
          ctx.store.updateGuild(ctx.guild.id, (guild) => { guild.boost.message = value; return guild; });
        ...
        if (action === 'test' || action === 'preview') {
          const member = await ctx.getMember('target', 1) || ctx.member;
          const textValue = fillTemplate(ctx.guildConfig.boost.message, { user: member.toString(), userTag: member.user.tag, server: ctx.guild.name, boostCount: ctx.guild.premiumSubscriptionCount || 0, boostTier: ctx.guild.premiumTier || 0 });
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, ctx.guildConfig.boost.title || '🚀 New boost', textValue)] });
        }
        return ctx.invalidUsage(`Examples: \`${ctx.prefix}boost view\`, \`${ctx.prefix}boost on\`, \`${ctx.prefix}boost channel #boosts\`, \`${ctx.prefix}boost test\`.`);
      }
    }),
    ...
    makeSimpleCommand({
      ...
      aliases: ['helpme', 'mp'],
      category: 'Support',
      description: 'Open support via DMs',
      ...
      async execute(ctx) {
        const text = ctx.interaction ? ctx.interaction.options.getString('message') : ctx.getRest(0);
        if (!ctx.guild) {
        ...
        if (!ctx.guildConfig.support.enabled || !ctx.guildConfig.support.channelId) {
          return ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '📨 Support not configured', 'A manager must run `+supporttoggle on` and `+setsupportchannel #channel` first.')] });
        }
        if (ctx.message) await ctx.message.delete().catch(() => null);
        ...
        if (text) {
          ...
          if (relayChannel?.isTextBased?.()) {
            const forwarded = await relayChannel.send({
              ...
              embeds: [baseEmbed(ctx.guildConfig, `📨 Support DM • ${ctx.user.tag}`, text)
                .addFields(
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        const attachmentUrls = getCommandAttachmentUrls(ctx);
        const imageUrl = getCommandImageUrl(ctx);
        if (!user || (!text && !attachmentUrls.length)) return ctx.invalidUsage('You can also attach an image or file to your reply.');
        ...
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '📨 Support reply', `Reply sent to **${user.tag}**.${attachmentUrls.length ? `
Attachments: **${attachmentUrls.length}**` : ''}`)] });
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        if (!target.giveaway) return ctx.invalidUsage('Use the giveaway ID, message ID, full giveaway message link, or reply to the giveaway message.');
        ctx.store.updateGuild(ctx.guild.id, (guild) => {
        ...
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '🎁 Giveaway', `Giveaway **${target.id}** (${target.giveaway.prize}) will end on the next check.`)] });
      }
    }),
    makeSimpleCommand({
      name: 'greroll',
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        const target = await resolveGiveawayEntry(ctx, ctx.getText('id', 0));
        if (!target.giveaway || target.giveaway.paused) return ctx.invalidUsage('Use an active giveaway ID / message link, not a paused one.');
        ctx.store.updateGuild(ctx.guild.id, (guild) => {
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        const target = await resolveGiveawayEntry(ctx, ctx.getText('id', 0));
        if (!target.giveaway || !target.giveaway.paused) return ctx.invalidUsage('Use a paused giveaway ID / message link, or reply to the paused giveaway message.');
        ctx.store.updateGuild(ctx.guild.id, (guild) => {
          const g = guild.giveaways[target.id];
          if (g) {
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        ...
        if (!target.giveaway || !prize) return ctx.invalidUsage('Example: `+gedit <giveaway-id-or-link> Nitro Basic`');
    ...
    makeSimpleCommand({
      ...
      async execute(ctx) {
        const entries = Object.values(ctx.guildConfig.giveaways || {});
        const desc = entries.length
          ? entries.map((g) => `**${g.id}** • ${g.prize}
↳ message: \`${g.messageId || 'none'}\` • winners: ${g.winners} • participants: ${(g.participants || []).length} • ended: ${g.ended ? 'yes' : 'no'} • paused: ${g.paused ? 'yes' : 'no'}`).join('\n\n')
          : 'No giveaways saved.';
        await ctx.reply({ embeds: [baseEmbed(ctx.guildConfig, '🎁 Giveaways', desc)] });
      }
    }),
  ...
  if (safePage === 'setup') {
    embed
      ...
      .addFields(
        ...
        {
          name: 'Useful commands',
          value: '`+config` • `+help setup` • `+setupcheck` • `+logs` • `+dashboard automation`',
          inline: false
        }
      );
  }
